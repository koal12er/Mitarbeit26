...
-- A65
select e.ENAME, d.DNAME from emps e join depts d on e.dept_id=d.DEPTNO where d.LOC='Chicago';

-- A66
select e.dept_id, e.JOB from emps e where e.SAL=(select MAX(SAL) from emps where JOB=e.JOB group by JOB)group by e.JOB;
select e.dept_id, e.JOB from emps e join (select MAX(SAL) as MAXSAL, JOB from emps group by JOB) a on e.JOB=a.JOB where e.SAL = a.MAXSAL;

-- A67
select e.dept_id, e.job from emps e join (select JOB, AVG(SAL) as AVGSAL from emps group by JOB) a on e.JOB=a.JOB where a.AVGSAL*1.1<=e.SAL;

-- A68
select e.SAL,e.ENAME,a.SAL,a.ENAME,((a.SAL-e.SAL)/e.SAL*100) from emps e join emps a on e.sal*1.5<=a.sal;
...
-- A70
create view FIL20 as select ENAME,JOB,id from emps where dept_id=20;
select * from FIL20 where JOB='Clerk';

-- A71
create view A71 as select MAX(SAL), MIN(SAL), AVG(SAL) as AVGSAL, dept_id from emps group by dept_id;
select * from A71;
...
select e.ename,
       ...
       e.sal,
       a.avgsal,
       ((e.sal - a.avgsal) / a.avgsal * 100) AS prozent,
       ABS(e.sal - a.avgsal) as diff
from emps e
         join A71 a on e.dept_id = a.dept_id
order by prozent;

select * from A72;

-- A73
create view ZAHLUNG as
select e.ENAME, e.JOB, e.SAL as Monatsgehalt, e.SAL*14 as Jahresgehalt, a.DNAME, a.LOC
from emps e join depts a on e.dept_id=a.DEPTNO;

-- A74
select SUM(Jahresgehalt), LOC from ZAHLUNG group by LOC;

-- A75
create table EMP as select * from emps e join depts d on e.dept_id = d.DEPTNO;